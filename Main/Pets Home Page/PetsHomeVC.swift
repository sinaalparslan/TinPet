...
class PetsHomeVC: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        // segment kontrol index daha önce define ettiğim indexten aldım  titleımıza segment kontorlü koydum frame ayarladım
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if selectedIndex == 0 {
            ...
            (selectedPage as? PetsListVC)?.animalList = getAnimalList()// selected page'in animal listesini set ettik
            selectedPage?.navigationItem.leftBarButtonItem = UIBarButtonItem(
                image: UIImage(systemName: "chevron.backward.circle"),
                style: .plain, target: self, action: #selector(actionBackRootView(_:)))
    ...
    @objc func actionBackRootView(_ sender: UIButton) {
