...
class RangeSlider: UIControl {
	...
	var valueChangeClosure: ((_ minValue: Int, _ maxValue: Int) -> Void)?
...
func updateUI() {
        ...
        leftTextLayer.frame = CGRect(x: leftHandleLayer.frame.minX - 0.5 * (kTextWidth - leftHandleLayer.frame.width),
                                     y: leftHandleLayer.frame.minY - kTextHeight, width: kTextWidth, height: kTextHeight)
        rightTextLayer.frame = CGRect(x: rightHandleLayer.frame.minX - 0.5 * (kTextWidth - leftHandleLayer.frame.width),
                                      y: leftTextLayer.frame.minY, width: kTextWidth, height: kTextHeight)
...
func setCurrentValue(left: Int, right: Int) {
		...
		let kTextWidth: CGFloat = 50
		let kTextHeight: CGFloat = 20
		leftTextLayer.frame = CGRect(x: leftHandleLayer.frame.minX - 0.5 * (
            kTextWidth - leftHandleLayer.frame.width),
                                     y: leftHandleLayer.frame.minY - kTextHeight, width: kTextWidth, height: kTextHeight)

		rightTextLayer.frame = CGRect(x: rightHandleLayer.frame.minX - 0.5 * (
            kTextWidth - leftHandleLayer.frame.width),
                                      y: leftTextLayer.frame.minY, width: kTextWidth, height: kTextHeight)
...
private func updateHighlightedBar() {
		...
		highlightedImageView.frame = CGRect(
            x: leftHandleLayer.frame.maxX - 0.5 * handleWidth, y: 0.5 * ( frame.height - barHeight ),
            width: rightHandleLayer.frame.minX - leftHandleLayer.frame.maxX + handleWidth, height: barHeight)
...
override func continueTracking(_ touch: UITouch, with event: UIEvent?) -> Bool {
		...
		if isLeftSelected {
			...
			leftHandleLayer.frame.origin.x = max(leftHandleLayer.frame.origin.x + deltaLocation,
                                                 normalbackImageView.frame.minX + 0.5 * handleWidth - leftHandleLayer.frame.width)
			leftHandleLayer.frame.origin.x = min(leftHandleLayer.frame.origin.x,
                                                 rightHandleLayer.frame.origin.x - leftHandleLayer.frame.width)
			...
			CATransaction.begin()
			CATransaction.setDisableActions(true)
			rightHandleLayer.frame.origin.x = min(rightHandleLayer.frame.origin.x + deltaLocation,
                                                  frame.width - rightHandleLayer.frame.width)
			rightHandleLayer.frame.origin.x = max(rightHandleLayer.frame.origin.x,
                                                  leftHandleLayer.frame.origin.x + leftHandleLayer.frame.width)
